...
4. **Reboot** — only when the new generation changes `kernel` or `init`, and only when `allowReboot = true` (opt-in, e.g. hp-240). The reboot then waits out a `rebootDelayMinutes` countdown (default 60): a critical notification announces the deadline, and the reboot is cancelled by clicking Reporter/Postpone on it or by creating `/var/lib/nixos-auto-update/postpone-reboot` (e.g. `sudo touch …`) — the marker is consumed and a "reboot manually" notice is emitted instead. Only markers created once the deadline is announced count: stale ones (forgotten manual touch, late click on an old notice) are dropped. Each milestone (deadline, imminent, postponed) is also broadcast with `wall` to all logged-in terminals (including terminal emulators), so CLI sessions see it too — `notify-send` alone only reaches graphical ones. Queued (headless) reboot notices are deliberately actionless: a late click must never decide anything. Otherwise a "reboot required / staged" notice is emitted — once per generation (see anti-spam).
