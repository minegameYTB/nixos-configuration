...
# run_case <delay> <precreate:0|1> <create_at_tick:0=never> <want_rc> <want_ticks> <desc>
...
run_case(){
  ...
  if MARKER="$marker" CREATE_AT="$create_at" CALLS="$T/ticks.log" bash -c "
    ...
    _await_reboot_window \"$delay\" \"\$MARKER\"
  " 2>/dev/null; then
    rc=0
  else
    rc=$?
  ...
  if [[ "$rc" == "$want_rc" && "$ticks" == "$want_ticks" ]]; then
    if [[ "$want_rc" == "1" && -e "$marker" ]]; then
      ko "$desc: postponed but marker not consumed"
    else
...
run_case 60 1 0 1 1 "pre-existing marker -> postponed at first check, marker consumed"
